...
class Recoder:
    ...
    def __init__(self, save_path, name):
        ...
        self.score_df = None
        self.score_save_path = os.path.join(save_path, f"{name}-score.csv")

        self.attention_df = None
        self.attention_save_path = os.path.join(save_path, f"{name}-attention.csv")
    ...
    def record_score_value(self, id_list, label_list, bag_num_list, score_list, epoch):
        ...
        self.score_df = (
            self.score_df.merge(temp_df) if self.score_df is not None else temp_df
        )
    ...
    def record_attention_value(self, patch_path_list, attention_value_list, epoch):
        ...
        self.attention_df = (
            self.attention_df.merge(temp_df)
            if self.attention_df is not None
            else temp_df
        )
