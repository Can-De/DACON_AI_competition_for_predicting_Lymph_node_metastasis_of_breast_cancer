...
def train_val_test_binary_class(task_type, epoch, model, data_loader, optimizer, recoder=None, writer=None, merge_method=None):
    ...
    if task_type == "train":
        ...
        for index, item in enumerate(data_loader, start=1):
            ...
            bag_tensor, label = item["bag_tensor"].to(torch.float32).to(device), item["label"].to(torch.int32).to(device)
            clinical_data = item["clinical_data"][0].to(torch.float32).to(device) if "clinical_data" in item else None
